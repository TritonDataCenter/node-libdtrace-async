...
static int dta_bufhandler(const dtrace_bufdata_t *bufdata, void *arg);
...
dta_async_open(dta_hdl_t *dtap)
{
	...
	if (dtrace_handle_buffered(dtp, dta_bufhandler, dtap) == -1) {
		(void) snprintf(dtap->dta_errmsg, sizeof (dtap->dta_errmsg),
...
static int
dta_fini(shim_ctx_t *ctx, shim_args_t *args)
{
	/* XXX */
	return (TRUE);
}
...
static int
dta_bufhandler(const dtrace_bufdata_t *bufdata, void *arg)
{
	dta_hdl_t *dtap = arg;
...
static void
dta_do_aggwalk(shim_ctx_t *ctx, dta_hdl_t *dtap)
{
	dtrace_hdl_t *dtp = dtap->dta_dtrace;
	int rval;
...
initialize(shim_ctx_t *ctx, shim_val_t *exports, shim_val_t *module)
{
	shim_fspec_t funcs[] = {
		...
		SHIM_FS_FULL("fini", dta_fini, 0, NULL, 0),
